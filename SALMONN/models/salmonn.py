...
class SALMONN(nn.Module):
    ...
    def prompt_wrap(self, embeds, atts, prompt, multi_prompt=False):
        if prompt:
            if multi_prompt:
                ...
                embed_token = torch.tensor([0] * embeds.shape[1])[None, :].to(p_before_tokens.input_ids.device)
                token_ids = torch.cat([p_before_tokens.input_ids, embed_token, p_after_tokens.input_ids], dim=1)
            ...
            return wrapped_embeds, wrapped_atts, token_ids
        else:
            return embeds, atts

    def forward(self, samples, verbose=False, prompt=None):
        ...
        speech_embeds, speech_atts, token_ids = self.prompt_wrap(speech_embeds, speech_atts, prompt, multi_prompt=self.multi_prompt)
    ...
    def generate(self, samples, generate_cfg, prompts=None, dummy=None):
        ...
        if prompts is not None:
            speech_embeds, speech_atts, token_ids = self.prompt_wrap(speech_embeds, speech_atts, prompts, multi_prompt=True)
        ...
        outputs = self.llama_model.generate(
            ...
            max_new_tokens=generate_cfg.get("max_new_tokens", 15),
